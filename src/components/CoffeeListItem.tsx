...
export function CoffeeListItem({ src, tags, title, description, price }: { src: string; tags: string[]; title: string; description: string; price: number }) {
  ...
  return (
    <div className="text-center bg-stone-200 px-8 py-4 rounded-tr-3xl rounded-bl-3xl rounded-tl-md rounded-br-md">
      ...
      <div className="flex gap-2 justify-center mb-4">
        {tags.map((tag, i) => (
          <p key={i} className="text-sm font-medium px-2 rounded-md text-amber-700 bg-amber-100">{tag.toUpperCase()}</p>
        ))}
      </div>
