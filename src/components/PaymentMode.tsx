...
export function PaymentMode({ formData }: ComponenteProps) {
  ...
  const paymentItemCSS = 'flex gap-2 bg-gray-100 border-[1px] px-4 py-2 rounded-lg items-center justify-center hover:outline hover:outline-1 hover:outline-green-300'
  ...
  function selectPaymentMode(mode: number) {
    ...
    const newButton: HTMLButtonElement | null = document.querySelector(`.payment-mode button:nth-child(${mode})`);
    if (newButton) {
  ...
  return (
    <div className="bg-stone-200 p-10 rounded-tr-3xl rounded-bl-3xl rounded-tl-md rounded-br-md">
      ...
      <div className="payment-mode grid grid-cols-3 gap-4" role="group">
        <button type="button" onClick={() => selectPaymentMode(1)} className={paymentItemCSS}>
          <CreditCard size={32} className="text-green-700" />
          CARTÃO DE CRÉDITO
        </button>
        <button type="button" onClick={() => selectPaymentMode(2)} className={paymentItemCSS}>
          <Bank size={32} className="text-green-700" />
          CARTÃO DE DÉBITO
        </button>
        <button type="button" onClick={() => selectPaymentMode(3)} className={paymentItemCSS}>
